...
function TovarTarixiDetail() {
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto font-sans">
      <Link to="/tovarlar" className="text-blue-600 underline mb-4 block">⬅ Ortga qaytish</Link>
      <h2 className="text-3xl font-bold text-gray-800 mb-6">{sana} sanasidagi mahsulotlar</h2>

      {cards.map((card, index) => (
        <div
          key={index}
          className="border rounded-lg bg-white shadow-md mb-6 p-6"
        >
          {card.ism && (
          ...
          <table className="w-full text-sm text-left border-t border-gray-200">
            <thead>
              <tr className="bg-gray-100 text-gray-700">
                <th className="py-2 px-3 border-b">Tovar</th>
                <th className="py-2 px-3 border-b">Miqdor</th>
                <th className="py-2 px-3 border-b">Narx</th>
                <th className="py-2 px-3 border-b text-right">Umumiy</th>
              </tr>
            </thead>
            <tbody>
              {card.mahsulotlar.map((m, i) => (
                <tr key={i} className="hover:bg-gray-50">
                  <td className="py-2 px-3 border-b">{m.tovar}</td>
                  <td className="py-2 px-3 border-b">{m.miqdor} {m.birlik}</td>
                  <td className="py-2 px-3 border-b">{parseFloat(m.narx).toLocaleString()} so‘m</td>
